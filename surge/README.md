...
The audited VPS exposes Nginx over plain HTTP and keeps Sub-Store ports 3000 and
3001 on loopback. Do not weaken the existing Basic Auth or expose those ports.
The long-term managed-profile path is instead:

```text
Surge Mac/iOS
    -> Tailscale Funnel HTTPS (*.ts.net)
    -> path-restricted service on the always-on Mac mini
    -> restricted SSH local-forward
    -> VPS 127.0.0.1:3001
    -> token-validated Sub-Store file share
```

Port 3001 is the Sub-Store frontend entry point. Port 3000 is the backend API
and must not be the Funnel target. The Mac mini service must allow only the
exact `surge_v2` file-share path and reject the management UI, `/api`,
`/download` and every other path. The iPhone uses the public HTTPS Funnel URL
and does not need to run Tailscale.

The final Surge profile URL has this shape and must be copied from Sub-Store,
not assembled or committed manually:

```text
https://MAC-MINI.TAILNET.ts.net/share/file/surge_v2?token=RANDOM_READ_ONLY_TOKEN
```
...
the long random Sub-Store share token is the client credential. Neither value
